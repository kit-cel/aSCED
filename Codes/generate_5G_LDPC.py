...
def generate_5G_LDPC(BG_no, K_target, N_target, return_lifting_size = False):
    ...
    BG = pd.read_excel(file_name, 'Set %d (a = %d)' %(i+1, a), header=None).to_numpy()
    ...
    for i in range(M_b):
        for j in range(N_b):
            if BG[i,j] != -1:
                H[i*Z : (i+1)*Z, j*Z : (j+1)*Z] = np.roll(np.eye(Z, dtype=np.uint8), BG[i,j], axis=1)
    ...
    if num_punctured_bits > 0:
        ...
        P = np.concatenate([np.arange(2*Z), N_b*Z - num_punctured_bits + np.arange(num_punctured_bits)])
    else:
