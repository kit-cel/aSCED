...
if flag_aed:
    ...
    if bool_emulate_stopping:
        ensemble_decoder_config.set_mConvergedConfig(
            int(np.ceil(target_fraction_coverged_path * Z))
        )
...
if flag_asced_17:
    ...
    if bool_emulate_stopping:
        sim_asced_17.set_mConvergedConfig(
            int(np.ceil(target_fraction_coverged_path * 17))
        )
    # cfg.H = H
...
if flag_asced_31:
    ...
    if bool_emulate_stopping:
        asced_31_config.set_mConvergedConfig(
            int(np.ceil(target_fraction_coverged_path * 31))
        )
